...
- Grok Build finite one-shot Consult/Delegate with executable/version/hash checks, profiles, strict argv, saved-session environment/privacy enforcement, process-shared Linux SQLite host gating around actual spawn, local fairness/turn admission, process-tree cleanup, ledgering, and configurable direct-workspace or linked-worktree policy.
...
On Linux, the current source gate passes: `npm run check` succeeds and the Node suite reports **778 tests, 778 passed, 0 failed, and 0 skipped**. Provider-activation, Compatibility Watch transition, maximum-utilization receipt/dispatch, strengthened Continuity native authority, Electron bootstrap, Hermes reverse/blocker coverage, and the new Grok host gate use local fake/synthetic evidence. Separately, a 2026-08-23 disposable live acceptance proved Grok Build Delegate can write in a dirty primary workspace under the owner-authorized direct profile, preserve pre-existing owner work, and continue the same native Codex task; it predates and does not certify the new shared gate. Nous Flash and Pro Integrated each completed a Codex-owned file-tool turn with exact output. `npm run test:release-bundle` passes **23/23**, and `npm pack --dry-run --json` includes the intended 0.6.0 source/media while excluding `.working` coordination state. A loopback-only true-browser smoke exercised Continuity Rename and confirmed Promote against a synthetic local API: the renamed task refreshed, actionable pending recovery rendered, both controls disabled, the 390px panel had no horizontal overflow, no console warnings appeared, and no native task, Goal, recovery, or operation IDs entered the DOM. This is browser interaction evidence, not exact Linux or Windows native App Server authority. Earlier native Windows/two-host evidence remains provenance only and does not certify the current delta.
...
| Grok Build Consult/Delegate | Dedicated adapter plus prior live Linux direct dirty-primary write and same-native-task continuation acceptance; the new canonical SQLite gate, cross-process contention, TTL/epoch/revoke, spawn barrier, privacy, and billing fail-closed paths are offline-tested. | The new gated route has not made a live provider contact; Windows direct-workspace acceptance, live terminal-envelope compatibility, sustained fleet capacity, nested-agent accounting, consumer weighting, and entitlement drift remain unknown. |
...
- Grok Delegate defaults to an explicitly authorized supplied workspace, including primary/dirty/non-Git workspaces; optional linked-worktree/clean-start policy remains available. Nested agents remain inside the parent boundary.
