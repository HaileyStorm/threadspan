...
| Grok Build Consult/Delegate | Dedicated adapter plus live Linux direct dirty-primary write and same-native-task continuation acceptance; the default-off one-continuation exploration path remains offline-tested. | Windows direct-workspace acceptance, live terminal-envelope compatibility for exploration recovery, sustained fleet capacity, nested-agent accounting, consumer weighting, and entitlement drift. |
