...
test("incremental planning selects only requested components and optional profiles", async (t) => {
  ...
  assert.deepEqual(
    ...
    [
      "gpt-6-astra-default.config.toml",
      "gpt-5.6-default.config.toml",
      "spark.config.toml",
      "gpt-6-astra-1m.config.toml",
...
test("context profiles use exact defaults and enforce the 90 percent ceiling", () => {
  ...
  assert.throws(
    () => validateContextProfile("bad-effort", { model: "test", reasoningEffort: "ultra", contextWindow: 100, autoCompactTokenLimit: 90 }),
...
test("context profile upgrades retire only exact legacy Threadspan files and can restore them", async (t) => {
  const root = await temporaryRoot(t);
  const exactPath = join(root, "gpt-5.6-600k.config.toml");
  const modifiedPath = join(root, "gpt-5.6-1m.config.toml");
  const exact = "# Generated by Threadspan. Select with: codex --profile gpt-5.6-600k\nmodel = \"gpt-5.6-sol\"\nmodel_context_window = 600000\nmodel_auto_compact_token_limit = 480000\n";
  const modified = "# Generated by Threadspan. Select with: codex --profile gpt-5.6-1m\nmodel = \"owner-choice\"\nmodel_context_window = 1000000\nmodel_auto_compact_token_limit = 800000\n";
  await writeFile(exactPath, exact);
  await writeFile(modifiedPath, modified);

  const plan = createInstallerPlan({ installRoot: root, selection: ["context-profiles"], longContextProfiles: "all", planId: "retire-legacy-context" });
  const removal = plan.operations.find((operation) => operation.operationKind === "remove-managed-file");
  assert.equal(removal.relativePath, "gpt-5.6-600k.config.toml");
  assert.match(removal.expectedPreimageSha256, /^[0-9a-f]{64}$/);
  assert.equal(plan.exclusions.some((item) => item.relativePath === "gpt-5.6-1m.config.toml"), true);
  assert.match(plan.exclusions.find((item) => item.relativePath === "gpt-5.6-1m.config.toml").reason, /differs from the exact Threadspan-managed legacy content/);
  assert.match(previewInstallerPlan(plan).text, /gpt-5\.6-600k\.config\.toml \(remove exact retired managed file\)/);

  const ownerPath = join(root, "owner.config.toml");
  await writeFile(ownerPath, exact);
  const forged = structuredClone(plan);
  forged.operations.find((operation) => operation.operationKind === "remove-managed-file").relativePath = "owner.config.toml";
  forged.digest = computePlanDigest(forged);
  await assert.rejects(applyInstallerPlan(forged, { approvedDigest: forged.digest }), /Invalid managed file removal operation/);
  assert.equal(await readFile(ownerPath, "utf8"), exact);

  const result = await applyInstallerPlan(plan, { approvedDigest: plan.digest });
  await assert.rejects(readFile(exactPath, "utf8"), { code: "ENOENT" });
  assert.equal(await readFile(modifiedPath, "utf8"), modified);
  assert.match(await readFile(join(root, "gpt-6-astra-default.config.toml"), "utf8"), /model_context_window = 602000/);
  assert.equal(await validateInstallerAppliedState(plan), true);
  assert.deepEqual(await applyInstallerPlan(plan, { approvedDigest: plan.digest }), result);

  const uninstall = await createInstallerUninstallPlan(result.manifestPath, { planId: "restore-retired-context" });
  await applyInstallerUninstallPlan(uninstall, { approvedDigest: uninstall.digest });
  assert.equal(await readFile(exactPath, "utf8"), exact);
  assert.equal(await readFile(modifiedPath, "utf8"), modified);
  await assert.rejects(readFile(join(root, "gpt-6-astra-default.config.toml"), "utf8"), { code: "ENOENT" });

  const exactOneMillion = "# Generated by Threadspan. Select with: codex --profile gpt-5.6-1m\nmodel = \"gpt-5.6-sol\"\nmodel_context_window = 1000000\nmodel_auto_compact_token_limit = 800000\n";
  await writeFile(modifiedPath, exactOneMillion);
  const bothPlan = createInstallerPlan({ installRoot: root, selection: ["context-profiles"], planId: "retire-both-legacy-context" });
  assert.deepEqual(bothPlan.operations.filter((operation) => operation.operationKind === "remove-managed-file").map((operation) => operation.relativePath), [
    "gpt-5.6-600k.config.toml",
    "gpt-5.6-1m.config.toml",
  ]);
  await applyInstallerPlan(bothPlan, { approvedDigest: bothPlan.digest });
  await assert.rejects(readFile(exactPath, "utf8"), { code: "ENOENT" });
  await assert.rejects(readFile(modifiedPath, "utf8"), { code: "ENOENT" });
});

test("installer labels Astra 1M experimental until post-restart native acceptance", async () => {
  const source = await readFile(new URL("../ui/install.js", import.meta.url), "utf8");
  assert.match(source, /Astra 1M \(Experimental\)/);
  assert.match(source, /current native capture advertises 872K/);
  assert.match(source, /post-restart live acceptance/);
});
