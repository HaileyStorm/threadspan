...
export const RETIRED_CONTEXT_PROFILES = Object.freeze({
  "gpt-5.6-600k": "# Generated by Threadspan. Select with: codex --profile gpt-5.6-600k\nmodel = \"gpt-5.6-sol\"\nmodel_context_window = 600000\nmodel_auto_compact_token_limit = 480000\n",
  "gpt-5.6-1m": "# Generated by Threadspan. Select with: codex --profile gpt-5.6-1m\nmodel = \"gpt-5.6-sol\"\nmodel_context_window = 1000000\nmodel_auto_compact_token_limit = 800000\n",
});
...
export function createInstallerPlan(options) {
  ...
  for (const id of selectedComponents) {
    ...
    if (id === "context-profiles") {
      ...
      for (const name of ["gpt-6-astra-default", "gpt-5.6-default", "spark", ...profileNames]) {
      ...
      for (const [name, content] of Object.entries(RETIRED_CONTEXT_PROFILES)) {
        appendInspectedRetirement({ operations, unchanged, exclusions }, installRoot, {
          component: id,
          relativePath: `${name}.config.toml`,
          content,
        });
...
function appendInspectedRetirement(collections, installRoot, candidate) {
  ...
  try {
    ...
    if (error?.code !== "ENOENT") {
      collections.exclusions.push(excludedCandidate(candidate, `Retired target could not be inspected and was preserved: ${error.message}`).exclusion);
      return;
    }
  }
  if (!stats) {
    collections.unchanged.push({ component: candidate.component, relativePath: candidate.relativePath, reason: "retired-profile-absent" });
    return;
  }
  if (stats.isSymbolicLink() || !stats.isFile()) {
    collections.exclusions.push(excludedCandidate(candidate, "Retired target is not a regular managed file and was preserved.").exclusion);
    return;
  }
  let previous;
  try {
    ...
    collections.exclusions.push(excludedCandidate(candidate, `Retired target could not be read and was preserved: ${error.message}`).exclusion);
    return;
  }
  if (previous !== candidate.content) {
    collections.exclusions.push(excludedCandidate(candidate, "Retired Codex profile differs from the exact Threadspan-managed legacy content and was preserved.", previous).exclusion);
    return;
  }
  collections.operations.push({
    component: candidate.component,
    operationKind: "remove-managed-file",
    relativePath: candidate.relativePath,
    expectedPreimageSha256: sha256(previous),
    change: {
      kind: "remove",
      scope: `component:${candidate.component}`,
      reason: "Exact obsolete Threadspan-managed context profile is retired.",
 ...
 * @param {{model:string, reasoningEffort?:string, contextWindow:number, autoCompactTokenLimit:number}} profile Profile values.
 * @returns {string}
 */
export function renderContextProfile(name, profile) {
  validateContextProfile(name, profile);
  const reasoning = profile.reasoningEffort === undefined ? "" : `model_reasoning_effort = ${JSON.stringify(profile.reasoningEffort)}\n`;
  return `# Generated by Threadspan. Select with: codex --profile ${name}\nmodel = ${JSON.stringify(profile.model)}\n${reasoning}model_context_window = ${profile.contextWindow}\nmodel_auto_compact_token_limit = ${profile.autoCompactTokenLimit}\n`;
}
...
export function validateContextProfile(name, profile) {
  ...
  if (profile.reasoningEffort !== undefined && !["low", "medium", "high", "xhigh", "max"].includes(profile.reasoningEffort)) {
    throw new TypeError(`${name}: reasoningEffort is unsupported`);
  }
