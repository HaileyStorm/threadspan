...
const state={step:0,bootstrap:null,selected:new Set(),longContext:new Set(),plan:null,applied:null,session:null,resume:null,helper:null,donationShown:false,taskDisposition:"wait",taskIds:[],taskReceipt:null,activationFlow:false,activationRecoverClaimDigest:"",themeMode:normalizeThemeMode(document.documentElement.dataset.themeMode),hostTheme:null,voicePresets:[],voice:{selectedProfile:"technical-partner",profiles:[]}};
...
function renderComponents(){
  ...
  const writing=groups.writingTools.map(componentChoiceMarkup).join("");
  ...
  const writingDisclosure=writing?`<details class="writing-tools"><summary>Writing tools <span>${groups.writingTools.length}</span></summary><p id="writing-tools-help">Optional and off until you select it. Setup only adds a reviewed local descriptor.</p><div class="choice-list" aria-describedby="writing-tools-help">${writing}</div></details>`:"";
  content.innerHTML=`<div class="choice-list">${main}</div>${writingDisclosure}${disclosure}<div class="disposition"><strong>Optional long context:</strong><label title="Experimental: the current native capture advertises 872K; use after post-restart live acceptance"><input type="checkbox" data-long-context="gpt-6-astra-1m" ${state.longContext.has("gpt-6-astra-1m")?"checked":""}> Astra 1M (Experimental)</label></div>${state.selected.has("voice-profiles")?voiceSettingsMarkup():""}`;
  ...
  content.querySelectorAll(".help-tip").forEach(el=>el.addEventListener("click",event=>{event.preventDefault();event.stopPropagation();el.focus()}));
...
function renderReview(){
  ...
  if(p.kind==="threadspan-provider-activation"){
    content.innerHTML=`<div class="review"><pre class="preview">${esc(state.plan.preview.text)}</pre><aside class="evidence"><div><span>Exact route</span><strong>${esc(p.request.routeId)}</strong></div><div><span>Attempts</span><strong>1</strong></div><div><span>Plan digest</span><strong title="${p.digest}">${p.digest.slice(0,12)}…</strong></div><div><span>Acceptance cap</span><strong>${fmt(estimate.high)} tokens</strong></div></aside></div><section class="approval"><h2>Closed activation boundary</h2><p>No smart routing, fallback, takeover, retry, credential transfer, or Desktop/provider app lifecycle is authorized. Blocked selected providers remain visible but cannot execute.</p></section>`;return
  }
...
function renderApproval(){
  if(state.plan.plan.kind==="threadspan-provider-activation"){
    content.innerHTML=`<section class="approval"><h2>Exact provider request</h2><p>Enable only <code>${esc(state.plan.plan.request.providerId)}</code> for one live request on <code>${esc(state.plan.plan.request.routeId)}</code>. A failed or uncertain request is not retried.</p><label>Reviewed stale-claim recovery digest <input id="activation-recover-claim" autocomplete="off" value="${esc(state.activationRecoverClaimDigest)}" placeholder="Optional SHA-256 from a prior ownership error"></label><label><input id="write-approved" type="checkbox"> <span>Apply provider activation plan <code>${state.plan.plan.digest.slice(0,16)}…</code>.</span></label></section>`;const sync=()=>next.disabled=!$("write-approved").checked;$("write-approved").addEventListener("change",sync);$("activation-recover-claim").addEventListener("input",()=>{state.activationRecoverClaimDigest=$("activation-recover-claim").value.trim()});sync();return
  }
...
function renderProof(){
  ...
  const activation=r?.kind==="threadspan-provider-activation-receipt";
  const activationButton=pending&&!activation?'<section class="approval"><h2>Activate one provider</h2><p>Choose one exact mode/provider/account/model request. Authentication and runtime checks are explicit attestations; credential values never enter this window.</p><label>Provider <select id="activation-provider">'+activationProviders().map(id=>`<option value="${esc(id)}">${esc(id)}</option>`).join("")+'</select></label><label>Mode <select id="activation-mode"><option>consult</option><option>integrated</option><option>delegate</option></select></label><label>Model <input id="activation-model" autocomplete="off" placeholder="Exact non-auto model"></label><label>Account/profile reference <input id="activation-account" autocomplete="off" placeholder="Optional opaque account ID"></label><label><input id="activation-auth" type="checkbox"> Authentication is ready in provider-native or environment state.</label><label><input id="activation-runtime" type="checkbox"> Runtime/CLI/SDK prerequisite is ready.</label><button class="secondary" id="review-activation" type="button">Review exact activation</button></section>':"";
  content.innerHTML=(r?`<div class="choice-list"><div class="choice"><span>${pending?"…":"✓"}</span><span><strong>${activation?(r.status==="ready"?"Provider activation proved":"Provider activation blocked"):unchanged?"Installation already matched":preserved?"Existing settings preserved":pending?"Files and service lifecycle applied; activation remains pending":"Installation applied"}</strong><small>${activation?`Exact request ${esc(r.routeId||"")} finished ${esc(r.status||"blocked")}.`:unchanged?"No files or host state changed.":preserved?"No files changed; review the digest-bound exclusions before any future install.":pending?`Provider/runtime evidence is not yet ready${providerSummary?`: ${esc(providerSummary)}`:""}. Host surfaces remain ${esc(r.hostSurface?.status||"pending")}.`:`${esc(r.written?.length||0)} files changed. Rollback manifest retained.`}</small></span><span class="tier">${pending?"Pending":r.status==="blocked"?"Blocked":"Proved"}</span></div></div><pre class="preview">${esc(JSON.stringify(r,null,2))}</pre>${activationButton}`:`<p class="empty">No apply result is available.</p>`)+roadmapNote();
  $("review-activation")?.addEventListener("click",reviewProviderActivation);
}

function activationProviders(){return state.plan?.plan?.selectedProviderIds||state.applied?.providerEvidence?.map(item=>item.providerId)||[]}
async function reviewProviderActivation(){
  const providerId=$("activation-provider").value,mode=$("activation-mode").value,model=$("activation-model").value.trim(),accountId=$("activation-account").value.trim();if(!model||model==="auto")throw new Error("Enter one exact non-auto model");
  state.plan=await api("plan",{providerActivation:true,request:{providerId,mode,model,...(accountId?{accountId}: {})},readiness:{authReady:$("activation-auth").checked,runtimeReady:$("activation-runtime").checked,authRef:accountId?`account:${accountId}`:undefined}},"POST");state.activationFlow=true;state.step=3;addEvent("Activation","Exact one-attempt provider plan created");render()
}
...
next.addEventListener("click",async()=>{try{
  ...
  if(state.step===4){
    ...
    const activation=state.plan.plan.kind==="threadspan-provider-activation";
    const fresh=state.plan.plan.kind==="threadspan-fresh-install";state.applied=await api("apply",activation?{approvedDigest:state.plan.plan.digest,...(state.activationRecoverClaimDigest?{recoverClaimDigest:state.activationRecoverClaimDigest}:{})}:noChanges?{approvedDigest:state.plan.plan.digest}:{approvedDigest:state.plan.plan.digest,...(fresh?{}:{desktopClosureApproved:true}),manualTaskConfirmation:$("tasks-ready").checked},"POST");
    addEvent("Install",noChanges?"No changes required; approval prompts skipped":"Plan applied and rollback manifest written");
  }
  state.step++;render();
}catch(e){fail(e)}});
back.addEventListener("click",()=>{if(state.activationFlow)return;if(state.step>0){state.step=state.step===3&&state.plan?.plan?.hasChanges===false?1:state.step-1;render()}});
...
function groupInstallerComponents(){const ready=[],addProviders=[],writingTools=[];for(const component of installerComponents()){if(component.group==="writing-tools")writingTools.push(component);else if(!PROVIDER_COMPONENT_IDS.has(component.id)||componentReadiness(component)==="ready")ready.push(component);else addProviders.push(component)}return{ready,addProviders,writingTools}}
...
function componentChoiceMarkup(c){const readiness=PROVIDER_COMPONENT_IDS.has(c.id)?componentReadiness(c):null;const readinessText=c.availabilityLabel||readinessLabel(readiness);return `<label class="choice" title="${esc(c.description)}"${readiness?` data-readiness="${readiness}"`:""}><input type="checkbox" data-component="${esc(c.id)}" ${state.selected.has(c.id)?"checked":""}><span><strong>${esc(c.label)}</strong><small>${esc(c.description)}</small>${readiness?`<small class="readiness ${readiness}">Readiness: ${esc(readinessText)}</small>`:""}${c.id==="copy-naturalizer"?copyNaturalizerDetails(c):""}${c.id==="copy-check"?copyCheckDetails(c):""}${c.id==="codex-full-access"?'<small><strong>Full-access warning:</strong> explicit-only and separately confirmed. Existing tools may read and write files, execute commands, and use the network without approvals. This does not install or enable new tools, apps, plugins, or servers; existing per-tool overrides may still apply.</small>':""}</span>${surfaceTier(c.id)}</label>`}
function copyNaturalizerDetails(c){return `<small class="copy-features"><span class="help-tip" tabindex="0" aria-label="Local checks: ${esc(c.localHeuristicsTooltip)}" data-tooltip="${esc(c.localHeuristicsTooltip)}">Local checks</span><span class="help-tip" tabindex="0" aria-label="Optional rewrite: ${esc(c.configuredRewriteTooltip)}" data-tooltip="${esc(c.configuredRewriteTooltip)}">Optional rewrite</span></small><small class="copy-disclaimer">${esc(c.localDisclaimer)}</small>`}
function copyCheckDetails(c){return `<small class="copy-features"><span class="help-tip" tabindex="0" aria-label="Destination: ${esc(c.destinationTooltip)}" data-tooltip="${esc(c.destinationTooltip)}">Destination</span><span class="help-tip" tabindex="0" aria-label="Payload: ${esc(c.payloadTooltip)}" data-tooltip="${esc(c.payloadTooltip)}">Payload</span><span class="help-tip" tabindex="0" aria-label="Retention: ${esc(c.retentionTooltip)}" data-tooltip="${esc(c.retentionTooltip)}">Retention</span><span class="help-tip" tabindex="0" aria-label="Trial: ${esc(c.trialTooltip)}" data-tooltip="${esc(c.trialTooltip)}">Trial</span><span class="help-tip" tabindex="0" aria-label="Partnership: ${esc(c.partnershipTooltip)}" data-tooltip="${esc(c.partnershipTooltip)}">No partnership</span></small><small class="copy-disclaimer">${esc(c.detectorDisclaimer)}</small>`}
...
function surfaceTier(id){const map={"grok-build":"Enhanced",cursor:"Standard","codex-native":"Primary","claude-code":"Preview","maximum-utilization":"Optional","copy-naturalizer":"Optional","copy-check":"Optional"};return map[id]?`<span class="tier">${map[id]}</span>`:"<span></span>"}
