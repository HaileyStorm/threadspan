...
Threadspan reads task, status, name, lineage, and Goal state through source-bound Codex App Server process receipts plus a private binding digest over each exact request. A private schema-v2 state file serializes cooperative Threadspan writers with an exact-owner host/process claim and revision/digest conflict checks; it does not claim hostile same-user kernel-level compare-and-swap. Legacy unresolved dispatches migrate to explicit non-replayable recovery. The HUD never edits Codex databases.

Promotion and rollover remain requests to the installed Continuity supervisor; Threadspan does not claim a conditional-dispatch authority that App Server does not expose. It journals the stable operation and recovery identity before `turn/start`. Any uncertainty after that journal is `dispatch-indeterminate` and is never automatically replayed. Acceptance requires exactly one `continuity:worker:<operation>:rw` successor, a fresh inactive read-back and archive evidence for the exact predecessor, and Goal-free parity or exact available objective/status/accounting parity. Unsupported or ambiguous native fields stay visible and keep controls disabled.

Raw thread, Goal, turn, operation, recovery, receipt, prompt, and objective values remain private. The public service and browser adapter use closed projections with only bounded labels plus short-lived opaque task/operation handles. These guarantees are offline-tested with fake App Server processes; exact Linux and Windows native-build authority remains a separate acceptance gate.
