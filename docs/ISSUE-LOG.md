...
| A broad Grok implementation prompt spent its turn budget researching and planning instead of returning a bounded patch. | Added an opt-in provider-neutral classifier plus Delegate-only Grok integration: reserve final/test turns before launch, trust only top-level terminal/error fields plus stderr (never valid-envelope `output_text`), require an exact nonempty session echo/repeated plan-read activity/unchanged Git, serialize physical Git-worktree identity across aliases/instances, bind child cwd/argv/environment to the canonical physical top-level, then issue one patch-first `--resume`. Attempt failures retain only available hashes and acceptance commands remain count/digest-only in lifecycle records. | Offline fakes prove the boundary, including symlink retarget resistance, structured auth/quota/rate/payment coverage with discussion controls, and one trusted enabled-Delegate nonzero max-turn exit. Linux/Windows live CLI canaries must confirm current top-level activity/session fields before enabling it; Consult, disabled Delegate, and ordinary/quota/rate/payment/auth/entitlement/malformed failures never auto-recover. |
...
## 2026-08-18 Desktop visibility correction
...
- **Fix:** The opt-in Delegate exploration loop now reserves a final/test budget, trusts only top-level provider terminal fields, requires exact session echo plus repeated structured plan/read activity and unchanged Git, serializes the writable workspace, and permits exactly one sequential patch-first continuation through adapter-owned `--session-id`/`--resume`. Initial and recovery attempt identity is distinct under the same job/thread/workspace; later authentication/entitlement failure stays terminal with hashed evidence and never creates a third invocation.
